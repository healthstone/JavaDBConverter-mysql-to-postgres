...
public class TableDbcSpellFocusObjectTest extends BaseTest {
    private static final Logger logger = LoggerFactory.getLogger("TableDbcSpellFocusObjectTest");

    @Test
    @Tag("dbc_spellfocusobject")
    @DisplayName("dbc_spellfocusobject.sql")
    @Description("Перелив данных для dbc_spellfocusobject.sql")
    public void convertTableDbcSpellFocusObject() {
        Query<MysqlDbcSpellFocusObject> query = mysqlDBCDatabaseSession.createQuery("from MysqlDbcSpellFocusObject", MysqlDbcSpellFocusObject.class);
        List<MysqlDbcSpellFocusObject> mysqlTable = query.getResultList();
        ...
        try {
            for (int i = 0; i < mysqlTable.size(); i++) {
                MysqlDbcSpellFocusObject mysql = mysqlTable.get(i);
                PgDbcSpellFocusObject pg = new PgDbcSpellFocusObject();
